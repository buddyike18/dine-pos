...
import { FullScreenModeContainer } from "../../../src/design-system";
...
export default function OrderingModeScreen() {
  ...
  const safeTableId = typeof tableId === "string" ? tableId.trim() : "";
  ...
  const isTableUnavailable = !hasValidTableId || !table;
  ...
  async function selectMenuItem(item: PosMenuItem) {
    if (isTableUnavailable) {
      setSubmitError("Table unavailable. Return to floorboard and try again.");
      return;
    }
  ...
  async function submitOrder() {
    if (cartItems.length === 0 || isSubmittingOrder || isTableUnavailable) {
      if (isTableUnavailable) {
        setSubmitError("Table unavailable. Return to floorboard and try again.");
      }
      return;
                      ...
                      >
                        {rowItems.map((item) => (
                          <Pressable
                            ...
                            disabled={
                              isTableUnavailable || !item.available
                            }
                            style={({ pressed }) => ({
                              flex: 1,
                              borderWidth: 1,
